...
class account_move_line(models.Model):
        ...
        for line in self:
            if line.ref:
                if line.date_maturity:
                    result.append((line.id, (line.move_id.name or '') + '(' + line.ref + ') - ' + line.date_maturity))
                else:
                    result.append((line.id, (line.move_id.name or '') + '(' + line.ref + ')'))
            elif line.date_maturity:
                result.append((line.id, line.move_id.name + ' - ' + line.date_maturity))
            else:
    ...
    def default_get(self, vals):
        res = super(account_move_line, self).default_get(vals)

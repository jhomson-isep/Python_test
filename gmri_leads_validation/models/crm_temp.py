...
class crm_temp(models.Model):
    ...
    def lead_cron(self):
        ...
        for lead in temp_lead_obj:
            ...
            try:
                self._produccion12(lead)
            except Exception as e:
                logger.info('Error en esta Iniciativa:')
                logger.info(e)
        #self.poner_partner()
    ...
    @api.multi
    def validar_lead(self):
        temp_lead_obj = self.env['crm.temp'].search([('check', '=', False), ('check_error', '=', False)], limit=50)
        for lead in temp_lead_obj:
            self.ajustar_lead(lead)
            logger.info('INICIATIVA ENTRANTE:')
            logger.info(lead)
            if lead.cod_curso=='curso' and lead.cod_sede=='codigo':
                self.cerrar_lead()
            else:
                self._check_lead(lead)
    ...
    def validate_lead(self):
        for lead in self:
            ...
            logger.info('INICIATIVA ENTRANTE:')
            logger.info(lead)
            if lead.cod_curso=='curso' and lead.codigo=='codigo':
                self.cerrar_lead()
            else:
                self._check_lead(lead)
        ...
        try:
            ...
            if lead.pais != '-':
                if lead.pais != 'Otro':
                    ...
                    if lead.pais.encode('utf-8') not in self.rel_country:
                        ...
                        logger.info("----------2-----------")
                        country_id = self.env['res.country'].search([
                            ('name', '=', self.rel_country[pais])],limit=1)
            ...
            if lead.cod_sede != '-':
                ...
                if lead.cod_sede != ' ':
                    ...
                    logger.info("TENGO COD SEDE 2: %s", lead.cod_sede)
                else:
                ...
                if tipocurso != ' ' and tipocurso != '':
                    ...
                    if prod_code:
                        if len(prod_code) > 1:
                            ...
                            logger.info("Comprobación: %s != %s", tipocurso.capitalize(
                            ), prod_code.tipodecurso.capitalize())
            ...
            logger.info("Cod Area: %s", lead.cod_area)
            ...
            curso_init = ''
            logger.info("Tipo de Curso : %s", lead.cod_tipo_curso)
            logger.info("prod_id : %s", prod_code)
            if lead.cod_tipo_curso == ' ' and prod_id.tipodecurso != ' ':
                if prod_code.tipodecurso == 'master':
                    curso_init = 'MT'
                elif prod_code.tipodecurso == 'curso':
                    curso_init = 'CU'
                elif prod_code.tipodecurso == 'pgrado':
                    curso_init = 'CU'
            elif prod_code.tipodecurso == '':
            ...
            logger.info("Equipo de Ventas: %s", id_equipo_ventas)
            logger.info("Siguente paso 10: %s", prod_code)
            ...
            if id_equipo_ventas.id == 22:
                nombre_sede = 'MDR'
                logger.info("1: %s", nombre_sede)
            else:
                nombre_sede = id_equipo_ventas.code
                logger.info("1-1: %s y equipo ventas %s", nombre_sede, id_equipo_ventas)
            ...
            logger.info("lead name: %s", lead.name)

            if not(id_equipo_ventas):
                logger.info ("NO TENGO EQUIPO DE VENTAS")
                if company_id.id ==1:
                    logger.info ("Companñia ISEP")
                    id_equipo_ventas = self.env['crm.team'].browse(7)
                    nombre_sede=id_equipo_ventas.code
                else:
                    logger.info ("Companñia ISED")
                    id_equipo_ventas =self.env['crm.team'].browse(8)
                    nombre_sede=id_equipo_ventas.code

            try:
                logger.info("dentro try")
                lead_name = prod_code.default_code[:2] + \
                    nombre_sede + ' - ' + lead.email
            except:
                logger.info("error")
                logger.info("1: %s", codigo_producto)
                logger.info("2: %s", nombre_sede)
                logger.info("3: %s", lead.email)
            ...
            logger.info("VALORES: %s y %s", prod_code, prot_prod)
            if prod_code or prot_prod:
                logger.info("UNO")
                if country_id or llamada_pais:
                    ...
                    if id_sede and company_id:
                        logger.info("TRES")
                        logger.info('cliente_obj: %s',cliente_obj.id)
                        logger.info('nombre: %s',lead.nombre + ' ' + lead.apellidos)
                        logger.info('phone: %s',lead.phone)
                        logger.info('descripcion: %s',lead.descripcion)
                        logger.info('sexo: %s',lead.sexo)
                        logger.info('estudios_finalizados: %s',lead.estudios_finalizados)
                        logger.info('cp: %s',lead.cp)
                        logger.info('country_id: %s',country_id.id)
                        logger.info('state_id: %s',state_id.id)
                        logger.info('lead: %s',lead.ciudad)
                        logger.info('nac: %s',nac)
                        logger.info('email: %s',lead.email)
                        logger.info('lane: %s',lead.lane)
                        logger.info('company_id: %s',company_id.id)
                        logger.info('lead_name: %s',lead_name)
                        logger.info('date: %s',date)
                        logger.info('codigo_producto: %s',codigo_producto)
                        logger.info('sede_llamada: %s',sede_llamada)
                        logger.info('cod_modalidad: %s',lead.cod_modalidad)
                        logger.info('cod_area: %s',lead.cod_area)
                        logger.info('estudios_finalizados: %s',lead.estudios_finalizados)
                        logger.info('campanya: %s',lead.campanya)
                        logger.info('content: %s',lead.content)
                        logger.info('medium: %s',lead.medium)
                        logger.info('utma: %s',lead.utma)
                        logger.info('source: %s',source)

                        logger.info('curso_init: %s',curso_init)
                        logger.info('user_id: %s',user_id.id)
                        #logger.info('id_sede: %s',id_sede.id)
                        logger.info('id_equipo_ventas: %s',id_equipo_ventas.id)
                        logger.info('prod_id: %s',prod_code.id)
                        logger.info('area: %s',area.id)
                        #logger.info('curso: %s',curso.id)
                        logger.info('eltipodecurso: %s',eltipodecurso)
                        logger.info('grupoduplicado: %s',grupoduplicado)
                        logger.info('numdups: %s',numdups)
                        logger.info('precontacto: %s',precontacto)                        
                        ...
                        logger.info("INICIATIVA %s", lead.iniciativa_id)
    ...
    def _traspasar_lead(self, lead):
        try:
            ...
            try:
                lead_name = prod_id.default_code[:2] + \
                    nombre_sede + ' - ' + lead.email
            except:
    ...
    def _produccion12(self, lead):
        ...
        if lead.pais != '-':
            if lead.pais != 'Otro':
                ...
                try:
                    if lead.pais.encode('utf-8') not in self.rel_country:
                        logger.info("----------1-----------")
                        country_id = self.env['res.country'].search(
                            [('name', '=', lead.pais), ('code', '!=', ''), ('create_uid', '!=', None)])
                    else:
                        logger.info("----------2-----------")
                        country_id = self.env['res.country'].search(
                            [('name', '=', self.rel_country[pais])])
                    ...
                    if not country_id:
                        logger.info("----------3-----------")
                        lead.pais= lead.pais.upper()
                        logger.info("----------3----------- , %s", lead.pais)
                        country_id = self.env['res.country'].search(
                            [('code', 'ilike', lead.pais), ('create_uid', '!=', None)])
                        logger.info("----------3----------- , %s", country_id )

                    if not country_id:
                        logger.info("----------4-----------")
                        country_id = self.env['res.country'].search(
                            [('name', 'ilike', lead.pais), ('create_uid', '!=', None), ])
                except:
                    country_id = self.env['res.country'].browse(68)
            else:
                llamada_pais = True
            ...
            if lead.phone[:2] == '52':
                country_id = self.env['res.country'].search(
                    [('code', '=', 'MX'), ('create_uid', '!=', None)])
            elif lead.phone[:2] == '57':
                country_id = self.env['res.country'].search(
                    [('code', '=', 'CO'), ('create_uid', '!=', None)])
            ...
            if lead.cod_sede != ' ':
                ...
                if country_id.code in ['CO', 'MX'] and lead.cod_modalidad != 'PRS':
                    ...
                    try:
                        ...
                        company_id = self.env['res.company'].browse(1111)
                    except:
                        id_equipo_ventas = self.env['crm.team'].browse(7)
                    ...
                    if lead.cod_sede == 'ONL':
                        ...
                        if lead.codigo == 'ISEP':
                            id_equipo_ventas = self.env['crm.team'].browse(5)
                    if lead.cod_sede == 'MAD':
                        if lead.codigo == 'ISED':
                            id_equipo_ventas = self.env['crm.team'].browse(10)
                            company_id = self.env['res.company'].browse(4)

                        if lead.codigo == 'ISEP':
                            id_equipo_ventas = self.env['crm.team'].browse(4)
                    if lead.cod_sede == 'BIO':
                        if lead.codigo == 'ISED':
                            id_equipo_ventas = self.env['crm.team'].browse(12)
                            company_id = self.env['res.company'].browse(5)
                        if lead.codigo == 'ISEP':
                            id_equipo_ventas = self.env['crm.team'].browse(16)
                    if lead.cod_sede == 'VAL':
                        if lead.codigo == 'ISED':
                            id_equipo_ventas = self.env['crm.team'].browse(12)
                        if lead.codigo == 'ISEP':
                            id_equipo_ventas = self.env['crm.team'].browse(19)

                    if lead.cod_sede == 'ZAZ':
                        if lead.codigo == 'ISED':
                            id_equipo_ventas = self.env['crm.team'].browse(11)
                            company_id = self.env['res.company'].browse(6)
                        if lead.codigo == 'ISEP':
                            id_equipo_ventas = self.env['crm.team'].browse(17)

                    if lead.cod_sede == 'PAM':
                        if lead.codigo == 'ISED':
                            id_equipo_ventas = self.env['crm.team'].browse(24)
                            company_id = self.env['res.company'].browse(22)
                        if lead.codigo == 'ISEP':
                            id_equipo_ventas = self.env['crm.team'].browse(17)
            else:
                logger.info("NO TENGO COD SEDE")
        ...
        logger.info(" **************** CREACION DE PARTNER ")
        cliente_obj = self.env['res.partner'].search(
            [('email', '=', lead.email)])
        # Duplicidad de Contactos #
        contactonuevodup = "Nuevo"
        ...
        if not(partner_dup):
            logger.info("NO LOCALIZADO")
            logger.info(" **************** Cracion Parter NUEVO **********************")
            vals_partner = {
                'name': lead.nombre.capitalize() + ' ' + lead.apellidos.capitalize(),
                'phone': lead.phone,
                'mobile': lead.mobile,
            ...
            partner_obj = self.env['res.partner']
            cliente_obj = partner_obj.sudo().create(vals_partner)
            logger.info("CLiente %s",cliente_obj[0])

        else:
            logger.info("_>>>>>>>>>>>>>DUPLICADO  %s", partner_dup.user_id.id)
            precontacto = partner_dup.user_id.id
        ...
        logger.info(" **************** CREACION DE INICIATIVA **********************")
        date = datetime.now()
        if lead.curso == 'Becas conecta':
            lead.descripcion = 'Becas conecta'
        ...
        logger.info(
            'x_sexo %s', lead.sexo in self.sex and self.sex[lead.sexo] or '')
        ...
        logger.info('x_precontactonuevodup %s',  precontacto)


        lead_dup= self.env['crm.lead'].search([
            ('contact_name', '=', lead_name)],
            limit=1)

        if (lead_dup):
            lead.iniciativa_id = lead_dup.id
            logger.info("DUPLICADO INICIATIVA : %s", lead.iniciativa_id)
            if lead.iniciativa_id:
                lead.check = True
                lead.check_error = False
                lead.error_iniciativa = ''
            ...
            vals_lead = {
                ...
                'x_grupoduplicado': '',
                'x_numdups': 0,
                'x_precontactonuevodup12': precontacto
            }
    ...
    @api.multi
    def poner_partner(self):
        logger.info("Poner partner")
        temp_lead_obj = self.env['crm.lead'].search(
            [('id', '>', 200000000), ('partner_id', '=', None)], limit=100)
        for lead in temp_lead_obj:
            try:
                logger.info(lead.id)
                self.crear_partner(lead.id)
            except Exception as e:
                logger.info(e)

    @api.multi
    def crear_partner(self, id):
        logger.info("CREAR PARTNER PARA QUIEN NO TIENE")
        lead = self.env['crm.lead'].search([('id', '=', id)], limit=1)
        temporal = self.env['crm.temp'].search(
            [('iniciativa_id', '=', id)], limit=1)
        logger.info(lead)
        logger.info(temporal)
        logger.info(temporal.email)
        partner_dup = self.env['res.partner'].search([
            ('company_id', '=', lead.company_id.id),
            ('email', 'ilike', temporal.email)],
            limit=1)
        logger.info("partner_dumpo")
        logger.info(partner_dup)
        logger.info("partner_dumpo")
        if not(partner_dup):
            logger.info("NO LOCALIZADO")
            vals_partner = {
                'name': temporal.nombre.capitalize() + ' ' + temporal.apellidos.capitalize(),
                'phone': temporal.phone,
                'mobile': temporal.mobile,
            ...
            partner_obj = self.env['res.partner']
            cliente_obj = partner_obj.sudo().create(vals_partner)
            lead.partner_id = cliente_obj.id
        else:
            logger.info("LOCALIZADO")
            logger.info(partner_dup)
            logger.info(partner_dup.email)
            lead.partner_id = partner_dup.id

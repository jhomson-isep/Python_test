...
class OpStudentMergeWizard(models.TransientModel):
    ...
    def action_merge(self):
        ...
        if len(self.student_ids) == 1:
            raise ValidationError(
                _('Seleccione al menos dos o mas estudiantes para combinar!!'))
    ...
    def _merge_fields(self, student_original, students_to_merge):
        ...
        for student_m in students_to_merge:
            student_to_merge = self.env['op.student'].search(
                [('id', '=', student_m)], limit=1)
            ...
            if values['place_birth'] in ['', student_to_merge.place_birth]:
                values['place_birth'] = student_to_merge.place_birth
            if values['birth_date'] is not None or values[
                'birth_date'] == student_to_merge.birth_date:
            ...
            if values['mobile'] in [student_to_merge.mobile, '']:
                values['mobile'] = student_to_merge.mobile
            if values['email'] == '' or values[
                'email'] != student_to_merge.email:
                values['email'] = student_to_merge.email
            if values['document_number'] == '' or values[
                'document_number'] != student_to_merge.document_number:
                values['document_number'] = student_to_merge.document_number
            if values['visa_info'] != '' or values[
                'visa_info'] != student_to_merge.visa_info:
                values['visa_info'] = student_to_merge.visa_info
            if values['phone'] != '' or values[
                'phone'] == student_to_merge.phone:
